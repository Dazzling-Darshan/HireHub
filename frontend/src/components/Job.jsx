import { Bookmark, BookmarkCheck, MapPin, Sparkles, Calendar, User as UserIcon, Check } from "lucide-react";
...
const Job = ({ job }) => {
  ...
  const isSaved = !!user && Array.isArray(savedJobs) && savedJobs.some((j) => j._id === job?._id);
  const isApplied = !!user && Array.isArray(allAppliedJobs) && allAppliedJobs.some((app) => (app?.job?._id === job?._id || app?.job === job?._id));
  ...
  return (
    <div className="p-6 rounded-2xl bg-card border border-border hover:border-primary/50 hover:shadow-xl transition-all duration-300 group hover:-translate-y-1 flex flex-col justify-between h-full min-h-[360px]">
      <div>
        {/* Top row */}
        <div className="flex items-center justify-between mb-4">
          <span className="text-xs text-muted-foreground font-medium bg-muted/60 px-2.5 py-0.5 rounded-full">
            {daysAgoFunction(job?.createdAt)}
          </span>

          <div className="flex items-center gap-2">
            {skillMatch && skillMatch.percentage > 0 && (
              <span className={`text-[11px] px-2.5 py-0.5 rounded-full font-bold border flex items-center gap-1 shadow-sm ${skillMatch.colorClass}`}>
        ...
        <div className="mb-3">
          <h3 className="font-bold text-lg text-foreground mb-1.5 group-hover:text-primary transition-colors duration-300 line-clamp-1">
            {job?.title}
          </h3>
        ...
        {/* Recruiter & Expiry Metadata */}
        <div className="flex items-center justify-between text-xs text-muted-foreground mb-3 flex-wrap gap-2 py-1.5 border-y border-border/40">
          ...
          {job?.expiryDate && (
            <span className="flex items-center gap-1 font-medium text-amber-600 dark:text-amber-400">
              ...
              Last Date: {formatDeadline(job.expiryDate)}
            </span>
          )}
        </div>
      ...
      <div className="flex gap-3 pt-4 border-t border-border mt-auto">
        ...
        <Button
          ...
          onClick={isApplied ? () => navigate(`/description/${job._id}`) : handleSaveToggle}
        >
          {isApplied ? "✓ Applied" : isSaved ? "✓ Saved" : "Save Job"}
        </Button>
      </div>
    </div>
  );
